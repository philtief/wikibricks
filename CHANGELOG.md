...
- **`notebooks/promote_edges.py`** — when `edges_proposed` has no rows
  ...
  result sets short-circuit to `[]`. The post-v0.7.14 nightly was
  green only because there were 2 pending edges to consume on its
  first run; today's 06:00 UTC scheduled run hit the empty case and
  failed. Regression test added.
