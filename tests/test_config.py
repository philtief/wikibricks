...
def test_config_precedence(tmp_path: Path):
    ...
    config = load_config(
        ...
        environ={
            ...
            "WIKIBRICKS_DATABASE_PATH": str(tmp_path / "explicit.db"),
        },
    )

    assert config.database_path == tmp_path / "explicit.db"
...
def test_config_environment_values_and_cli_defaults(tmp_path: Path):
    ...
    config = load_config(
        ...
        environ={
            ...
            "WIKIBRICKS_DATABASE_PATH": str(tmp_path / "memory.db"),
            "WIKIBRICKS_SYNC_INTERVAL_HOURS": "168",
